...
在 `上篇文章`_ 中我们的模版引擎实现了变量和注释功能，同时在文章的最后我给大家留了一个
...
下面我们就一起来让我们的模版引擎的 ``for`` 语法支持 ``break`` 和 ``else`` （可以从 Github 上下载: `template2c.py`_ ）

.. 至于如何让我们都模版都 ``for`` 标签也支持 ``break`` 和 ``else`` 这个任务就
...
就这样我们的模版引擎对 ``for`` 的支持算是比较完善了。
至于生成的代码里的换行和空格暂时先不管，留待之后优化代码的时候再处理。

重构
-------

我们的 ``Template.render`` 方法代码随着功能的增加已经变成下面这样了:
...
之后的文章还将实现其他实用的模版语法，比如 ``include``, ``extends`` 模版继承等。

``include`` 的语法(item.html 是个独立的模版文件, list.html 中 include item.html):
...
``extends`` 的语法(base.html 是基础模版, child.html 继承 base.html 然后重载 base.html
中定义的 block):
